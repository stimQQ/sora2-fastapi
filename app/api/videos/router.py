...
async def create_animate_move_task(
    ...
    Transfers video motion to a static image.

    IMPORTANT: Credits are NOT deducted immediately. They will be deducted
    after successful task completion based on actual video duration.
    ...
    try:
        ...
        # Check if user has enough credits (soft check, no deduction)
        user_credits = current_user.get("credits", 0)
        if user_credits < estimated_credits:
            raise HTTPException(
                status_code=status.HTTP_402_PAYMENT_REQUIRED,
                detail=f"Insufficient credits. Estimated requirement: {estimated_credits} credits "
                       f"(for ~5s video). Available: {user_credits} credits. "
...
@router.post("/animate-mix", response_model=AnimateResponse)
async def create_animate_mix_task(
    request: AnimateRequest,
    background_tasks: BackgroundTasks,
    current_user: dict = Depends(get_current_user),
    api_key: str = Depends(verify_api_key),
    ...
    Replaces the character in a video with a person from an image.
    ...
    try:
        ...
        return AnimateResponse(
            success=True,
            task_id=task_id,
            message="Face swap animation task created successfully",
            estimated_time=150  # 2.5 minutes estimated
        )
